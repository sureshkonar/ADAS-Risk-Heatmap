...
Author: Suresh Konar
Purpose: OEM-style ADAS risk intelligence using accident density,
         weather multiplier, road-type weighting, and ADAS layers (AEB / LDW)
"""
...
# KaggleHub is optional – guarded import
try:
    import kagglehub
...
st.title("🚗 RoadSentinel – ADAS Heatmaps & Analysis")
st.caption(
    "OEM-style ADAS risk visualization using accident density, "
    "weather impact, and road-type intelligence"
)
...
with st.expander("ℹ️ About this tool", expanded=True):
    st.markdown("""
**RoadSentinel** helps ADAS engineers, testers, and planners visually analyze
**where and why** ADAS features face higher operational risk.

### ADAS Layers Explained
- **AEB – Autonomous Emergency Braking**
  - Activated in **very high-risk zones**
  - Focus: collision avoidance, pedestrian & vehicle braking

- **LDW – Lane Departure Warning**
  - Activated in **moderate to high-risk zones**
  - Focus: unintended lane drift prevention

### What influences risk?
- Accident density (historical)
- Weather severity (rain / fog)
- Road type (NH / SH / Urban)

This tool is **decision-support**, not real-time ADAS control.
""")
...
selected_city = st.sidebar.selectbox(
    "Select City",
    state_city_map[selected_state]
)

grid_size = st.sidebar.slider(
    "Grid Resolution (degrees)",
    0.01, 0.05, 0.02, step=0.005
)

enable_weather = st.sidebar.checkbox("Enable Weather Risk", True)
enable_road_weight = st.sidebar.checkbox("Enable Road-Type Weighting", True)
        ...
        try:
            ...
            if csvs:
                ...
                if lat_cols and lon_cols:
                    df = df[[lat_cols[0], lon_cols[0]]].dropna()
                    df.columns = ["lat", "lon"]
                    df["road_type"] = np.random.choice(
                        ["NH", "SH", "Urban"], size=len(df)
                    )
                    return df
        except Exception:
            pass

    # ---------- Fallback: Synthetic India geo ----------
    np.random.seed(42)
    n = 25000
...
if run:
    ...
    if selected_state != "All India":
        lat0, lon0 = city_center[selected_city]
        df = df[
            df["lat"].between(lat0 - 0.3, lat0 + 0.3) &
            df["lon"].between(lon0 - 0.3, lon0 + 0.3)
        ]
    else:
        lat0, lon0 = 22.0, 78.0
    ...
    grid = (
        ...
        .agg(
            lat=("lat", "mean"),
            lon=("lon", "mean"),
            road_type=("road_type", lambda x: x.mode()[0]),
            accidents=("lat", "count")
        )
        .reset_index()
    )
    ...
    if enable_road_weight:
        road_weight = {"NH": 1.2, "SH": 1.0, "Urban": 0.8}
        grid["risk"] *= grid["road_type"].map(road_weight)
        st.info("🛣 Road-type weighting applied")

    # =========================
    # AEB / LDW ZONES
    # =========================
    aeb = grid[grid["risk"] > 70]
    ldw = grid[(grid["risk"] > 50) & (grid["risk"] <= 70)]

    # =========================
    # LEGEND
    # =========================
    with st.expander("🎨 Risk & Layer Legend", expanded=True):
        st.markdown("""
    ### 🔥 Heatmap Color Meaning
    - 🟢 **Low Risk (0–30)** → Sparse accidents, low ADAS intervention
    - 🟡 **Moderate Risk (30–50)** → Increased vigilance required
    - 🟠 **High Risk (50–70)** → LDW relevance zone
    - 🔴 **Very High Risk (70+)** → AEB critical intervention zone

    ### 🚗 ADAS Feature Layers
    - 🟥 **AEB – Autonomous Emergency Braking**
    - Triggered in **very high collision-risk zones**
    - Focus: frontal crash prevention

    - 🟨 **LDW – Lane Departure Warning**
    - Triggered in **moderate-to-high risk zones**
    - Focus: unintended lane drift prevention

    ### 🛣 Road-Type Weighting
    - **NH (National Highway):** Higher speed → higher severity
    - **SH (State Highway):** Medium risk
    - **Urban Roads:** Lower speed but dense traffic
    """)
    ...
    tab1, tab2, tab3 = st.tabs([
        "🔥 Overall ADAS Risk",
        "🟥 AEB Risk (Autonomous Emergency Braking)",
        "🟨 LDW Risk (Lane Departure Warning)"
    ])

    def render_map(data, layers, zoom=10):
    ...
    # ANALYTICS SECTION
    # =========================
    st.subheader("📊 ADAS Risk Analytics")

    c1, c2, c3 = st.columns(3)
    c1.metric("Total Grid Cells", len(grid))
    c2.metric("AEB Risk Zones", len(aeb))
    c3.metric("LDW Risk Zones", len(ldw))

    # ---- AEB vs LDW Distribution ----
    aeb_df = aeb[["risk"]].assign(zone="AEB")
    ldw_df = ldw[["risk"]].assign(zone="LDW")

    dist_df = pd.concat([aeb_df, ldw_df])
    ...
    fig1 = px.histogram(
        ...
        nbins=20,
        barmode="overlay",
        title=f"AEB vs LDW Risk Distribution – {selected_city}",
        labels={"risk": "Risk Score", "zone": "ADAS Feature"}
    )
    ...
    road_fig = px.box(
        ...
        y="risk",
        color="road_type",
        title="Road-Type Contribution to ADAS Risk",
        labels={"road_type": "Road Type", "risk": "Risk Score"}
    )
    ...
    st.markdown("""
---
### ⚠️ Disclaimer & Data Transparency
- Accident data: Public datasets (Kaggle) or synthetic fallback
- Weather: Open-Meteo API (forecast-based)
- Risk scores are **relative**, not absolute
- This tool is **not a safety-certified ADAS system**

© 2026 Suresh Konar – RoadSentinel
""")
